...
export async function GET() {
  return NextResponse.json({
    ...
    histories: checkHistories,
  });
}

/**
 * 헬스체크 수행 API (서버에서 주기적으로 호출)
 */
export async function POST() {
  try {
    ...
    if (!endpointsEnv) {
      return NextResponse.json(
        { success: false, message: 'HEALTH_CHECK_ENDPOINTS 환경 변수가 설정되지 않았습니다.' },
        { status: 500 }
      );
    }
    ...
    if (endpoints.length === 0) {
      return NextResponse.json(
        { success: false, message: 'HEALTH_CHECK_ENDPOINTS에 유효한 엔드포인트가 없습니다.' },
        { status: 500 }
      );
    }
